...
one, or stopping it first on an agent that cannot steer — and defaults to both `mod+enter` and
`alt+enter`. `composer.queueFollowUp` parks the draft as a queued follow-up instead and defaults to
`mod+shift+enter`. Both ignore **Settings** → **General** → **Follow-up behavior**, which only
decides what plain `Enter` does. A config that already binds `composer.sendNow` keeps its own
shortcut: defaults are only filled in for commands your config does not mention.
See [Message composer](./composer.md).
