...
`composer.queueFollowUp` queues the composer draft as a follow-up for when the agent goes idle,
instead of sending it into the running turn, and defaults to `mod+shift+enter`. See
