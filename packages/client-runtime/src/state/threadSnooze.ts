 ...
 * such threads could never be snoozed.
 */
 ...
 * the thread from CLASSIFYING as snoozed, exactly like blocked work.
 */
 ...
 * that hiding would bury just the same. A running session IS snoozable —
 * snooze only affects visibility, never the agent. Client-side twin of the
 * server invariants so the UI can reject before a round trip.
 */
