...
import { SubscriptionUsageSnapshot } from "./subscriptionUsage.ts";
...
/**
 * Probe failures are reported per provider inside the snapshot, so this only
 * fails when the caller is not allowed to read the environment.
 */
...
export const WsOrchestrationGetActivityOutputRpc = Rpc.make(
  ORCHESTRATION_WS_METHODS.getActivityOutput,
  {
    payload: OrchestrationGetActivityOutputInput,
