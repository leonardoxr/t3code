...
import { canSnooze, effectiveSnoozed } from "@t3tools/client-runtime/state/thread-snooze";
...
/**
 * The per-thread action menu (pin, snooze, rename, copy, delete…) as
 ...
 *
 * Unlike the sidebar, snooze here never navigates away: the caller
...
export function useThreadActionMenu(input: {
  ...
  const { snoozeThread, unsnoozeThread, pinThread, unpinThread, archiveThread, deleteThread } =
    useThreadActions();
