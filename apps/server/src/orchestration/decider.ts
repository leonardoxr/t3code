...
// QUEUED_TURN_START_GRACE_MS in client-runtime threadSnooze.ts.
...
/**
 * Real activity spends a snooze's return ticket: starting a turn on a snoozed
 * thread is the user re-engaging, so the thread belongs back in the inbox now
 * rather than on Tuesday. Shared by `thread.turn.start` and queued-follow-up
 * dispatch, which must look identical to the rest of the system.
 */
const threadSnoozeResetEvents = Effect.fn("threadSnoozeResetEvents")(function* (input: {
...
> {
  switch (command.type) {
    ...
    case "thread.unsnooze": {
      ...
      // Idempotent by re-emission: waking a thread that is not snoozed lands
      // on the same null state without churning updatedAt.
    ...
    case "thread.pin": {
      ...
      // silently outranking them — a snooze's return ticket is spent, because
      // the thread is on top NOW, not on Tuesday.
    ...
    case "thread.unpin": {
      ...
      // Idempotent by re-emission: unpinning a thread that is not pinned
    ...
    case "thread.pin.reorder": {
      ...
      // Idempotent by re-emission: a duplicate drop on the same slot keeps
      // the existing updatedAt so it projects as a no-op.
    ...
    case "thread.turn.start": {
      ...
      return [
        ...(yield* threadSnoozeResetEvents({ thread: targetThread, command })),
        userMessageEvent,
